...
class SteamPublishProfileDialog(QDialog):
    def __init__(self, session, build_id: str, parent=None):
        ...
        self.profile = None # Initialize profile attribute
        self._load_or_initialize_profile()

        # Set window title based on whether it's new or existing
        if self.profile and self.session.is_modified(self.profile): # Check if it was just added
            ...
            # Optionally close immediately: self.reject()
    ...
    def _load_or_initialize_profile(self):
        ...
        try:
            self.profile = self.session.query(SteamPublishProfile).filter_by(build_id=self.build_id).one_or_none()
            if self.profile is None:
                ...
                self.profile = SteamPublishProfile(build_id=self.build_id)
    ...
    def _init_ui(self):
        ...
        self.project_combo.currentIndexChanged.connect(self._update_builder_path_display)
        ...
        self.description_input.setPlaceholderText("Optional: Live branch name, build notes, etc.")
        form_layout.addRow("Description:", self.description_input)

        # --- Builder Path (Read-Only Display) ---
        self.builder_path_display = QLineEdit()
        self.builder_path_display.setReadOnly(True)
        self.builder_path_display.setToolTip("Path derived from the selected project's archive directory.")
        form_layout.addRow("Builder Path:", self.builder_path_display)

        # --- Depots Table ---
        self.depots_table = QTableWidget(0, 3)
        self.depots_table.setHorizontalHeaderLabels(["Depot ID", "Path (Directory)", "Browse"])
        self.depots_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        self.depots_table.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection)
        # Adjust column widths (optional)
        self.depots_table.horizontalHeader().setStretchLastSection(False)
        self.depots_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
        self.depots_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        self.depots_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)
        ...
        self.steamcmd_path_input.setPlaceholderText("Path to the directory containing steamcmd.exe or steamcmd.sh")
        self.steamcmd_browse_button = QPushButton("Browse")
        self.steamcmd_browse_button.clicked.connect(self._browse_steamcmd_path)
        steamcmd_layout.addWidget(self.steamcmd_path_input)
        steamcmd_layout.addWidget(self.steamcmd_browse_button)
        ...
        self.auth_button = QPushButton("Setup/Refresh Steam Auth")
        self.auth_button.clicked.connect(self._open_auth_settings)
        ...
        self.cancel_button.clicked.connect(self.reject) # reject() is standard for Cancel
    ...
    def _populate_fields(self):
        ...
        if self.profile.id is not None: # Check if it's an existing profile (has an ID)
            ...
            if self.profile.project:
                project_index = self.project_combo.findData(self.profile.project.id) # Find by ID
                if project_index >= 0:
                    self.project_combo.setCurrentIndex(project_index)
                else:
                    # Handle case where saved project is no longer valid?
                    QMessageBox.warning(self, "Warning", f"Saved project '{self.profile.project.name}' not found. Please select a project.")

            # Select current auth
            if self.profile.steam_config:
                 auth_index = self.auth_combo.findData(self.profile.steam_config.id) # Find by ID
                 if auth_index >= 0:
                      self.auth_combo.setCurrentIndex(auth_index)
                 else:
                      QMessageBox.warning(self, "Warning", f"Saved Steam authentication '{self.profile.steam_config.username}' not found. Please select or set up authentication.")

            # Set other fields
            self.app_id_input.setValue(self.profile.app_id)
            self.description_input.setText(self.profile.description or "")
            self.steamcmd_path_input.setText(self.profile.steamcmd_path or "")
    ...
    def _load_projects(self):
        ...
        try:
            ...
            if not projects:
                 ...
                 QMessageBox.warning(self, "No Projects", "No projects found in the database. Please add projects first.")
            else:
                self.project_combo.addItem("Select a Project...", None) # Placeholder item
    ...
    def _update_builder_path_display(self):
        """Update the read-only builder path based on selected project."""
        project_id = self.project_combo.currentData()
        path = "N/A - Select a project"
        if project_id is not None:
            try:
                # Query the project again based on ID to get the object
                selected_project = self.session.query(Project).filter_by(id=project_id).one()
                path = selected_project.archive_directory or "N/A - Path not set for project"
    ...
    def _insert_depot_row(self, depot_id=None, depot_path=None):
        ...
        browse_button.clicked.connect(lambda checked=False, r=row: self._browse_depot_path(r))
    ...
    def _browse_depot_path(self, row):
        ...
        start_dir = current_path_item.text() if current_path_item and current_path_item.text() else "" # Start Browse from current path if set
    ...
    def _browse_steamcmd_path(self):
        """Open a directory dialog to select the SteamCMD path."""
        start_dir = self.steamcmd_path_input.text() or ""
        path = QFileDialog.getExistingDirectory(self, "Select SteamCMD Directory", start_dir)
        if path:
            self.steamcmd_path_input.setText(path)

    def _open_auth_settings(self):
        """Placeholder to open the actual Steam Auth settings/dialog."""
        # --- Replace this with your actual auth dialog logic ---
        print("DEBUG: Opening Steam Auth Settings (Placeholder)...")
        # Example:
        # auth_dialog = SteamAuthDialog(self.session, self) # Assuming you have this dialog
        # result = auth_dialog.exec()
        # if result == QDialog.DialogCode.Accepted:
        #     print("Auth settings potentially updated.")
        #     self._refresh_auth_options() # Refresh dropdown after changes
        # else:
        #     print("Auth settings cancelled.")

        # --- For now, just show info and refresh ---
        QMessageBox.information(self, "Steam Auth",
            "Placeholder: This would open the Steam authentication setup dialog.\n"
            "Refreshing auth list now.")
            ...
            if not auths:
                ...
                if selected_index != -1:
                    self.auth_combo.setCurrentIndex(selected_index)
                     ...
                     QMessageBox.warning(self, "Auth Not Found", f"Previously selected Steam account ID {current_auth_id} not found. Please re-select.")

        except Exception as e:
            QMessageBox.critical(self, "Database Error", f"Failed to load Steam authentications: {e}")
            self.auth_combo.setEnabled(False)
    ...
    def save_profile(self):
        ...
        if not steamcmd_path:
             QMessageBox.warning(self, "Validation Error", "Please specify the SteamCMD Path.")
             self.steamcmd_path_input.setFocus()
             return
        ...
        try:
            ...
            selected_project = self.session.query(Project).filter_by(id=selected_project_id).one()
            selected_auth = self.session.query(SteamConfig).filter_by(id=selected_auth_id).one()
            ...
            self.profile.steamcmd_path = steamcmd_path
